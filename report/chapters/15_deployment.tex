...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item \textbf{PWA service worker:} \texttt{next-pwa} (Workbox) generates \texttt{sw.js} and \texttt{workbox-*.js} during build. These are excluded in development (\texttt{disable: process.env.NODE\_ENV === 'development'}).
\end{itemize}
...
\hline
\texttt{THINGSPEAK\_CHANNEL\_ID} & Server & Never exposed to the browser \\
\texttt{THINGSPEAK\_READ\_API\_KEY} & Server & Read-only; no write risk \\
\texttt{NEXT\_PUBLIC\_MAPTILER\_KEY} & Client & Prefixed \texttt{NEXT\_PUBLIC\_}; included in browser bundle \\
\hline
\end{tabularx}
\end{table}
...
PWA is configured in \texttt{next.config.ts} using \texttt{@ducanh2912/next-pwa}:
...
The PWA manifest is defined in \texttt{app/manifest.ts} and specifies the app name, icons (\texttt{icon-192x192.png}, \texttt{icon-512x512.png}), theme colour, background colour, and display mode (\texttt{standalone}).
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  \item The new deployment is promoted to the production URL (e.g.\ \texttt{floodeye.vercel.app}).
