...
\textbf{Purpose:} Static typing for all source files (\texttt{.ts}, \texttt{.tsx}).\\
...
\textbf{Why chosen:} Tailwind v4's JIT (just-in-time) compiler generates minimal CSS bundles. The glassmorphism design language (\texttt{bg-white/40 backdrop-blur-xl border-white/60}) is cleanly expressed with utility classes without custom CSS files.
...
\textbf{Why chosen:} Framer Motion provides a declarative API (\texttt{motion.div}, \texttt{AnimatePresence}) that integrates directly with React's component tree.
...
\textbf{Role:} The model is defined in \texttt{Model/nb\_code.py} (extracted from \texttt{FloodMonitoring.ipynb}). Training runs in Google Colab with GPU acceleration.
...
\textbf{Integration:} The custom \texttt{AttentionLayer} is reimplemented in TypeScript and registered with \texttt{tf.serialization.registerClass()} before model loading.

\subsection{scikit-learn StandardScaler}
\textbf{Purpose:} Feature normalisation during training.\\
\textbf{Role:} Scaler parameters (mean and scale vectors) are exported to \texttt{scalers.json} and loaded by the browser at inference time to apply the same normalisation.

\subsection{TensorFlow.js Converter}
\textbf{Purpose:} Converts the trained Keras \texttt{.h5} model to TensorFlow.js LayersModel format (\texttt{model.json} + \texttt{group1-shard1of1.bin}).

\section{Backend Technologies}

\subsection{Next.js Route Handlers}
\textbf{Purpose:} Serverless API endpoints at \texttt{app/api/*/route.ts}.
...
\textbf{Framework:} Arduino (C++ sketch: \texttt{IOTcode/IOTcode.ino}).
