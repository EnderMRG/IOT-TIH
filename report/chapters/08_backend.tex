...
\section{ThingSpeak Client (\texttt{lib/\allowbreak{}thingspeak.\allowbreak{}ts})}
...
The cache is implemented using a global singleton pattern (\texttt{globalThis.\allowbreak{}\_\allowbreak{}\_\allowbreak{}thingspeakCache}) to persist across serverless function invocations within the same Node.js instance:
...
The \texttt{deriveDeviceStatus()} function checks whether the ESP32 is online by comparing the ThingSpeak \texttt{created\_\allowbreak{}at} timestamp against the current time. If the last upload was more than 60 seconds ago, \texttt{esp32Online} is set to \texttt{false}.

\section{API Route Handlers}

\subsection{\texttt{GET /\allowbreak{}api/\allowbreak{}sensors}}

Returns the single latest sensor reading as a \texttt{\{data,\allowbreak{} deviceStatus\}} JSON object. Uses a 15-second ISR revalidation window (\texttt{export const revalidate = 15}).

\subsection{\texttt{GET /\allowbreak{}api/\allowbreak{}history?range=\{range\}}}
...
\subsection{\texttt{GET /\allowbreak{}api/\allowbreak{}analytics?range=\{range\}}}
...
Returns: \texttt{\{range,\allowbreak{} count,\allowbreak{} analytics: \{temperature,\allowbreak{} humidity,\allowbreak{} pressure,\allowbreak{} altitude,\allowbreak{} distance\}\}}.

\subsection{\texttt{GET /\allowbreak{}api/\allowbreak{}environment-score}}

Fetches the latest reading and computes the Environmental Comfort Score from temperature and humidity using a weighted formula. Returns \texttt{\{score,\allowbreak{} status\}}.

\subsection{\texttt{POST /\allowbreak{}api/\allowbreak{}flood-predict}}

A structural placeholder for future server-side TensorFlow.js Node inference. In the current implementation it validates the request body (must contain a \texttt{history} array of at least 48 entries) and returns HTTP 501 (Not Implemented) with a message directing callers to use the client-side \texttt{lib/\allowbreak{}flood-model.\allowbreak{}ts} inference path.

\section{TelemetryProvider (\texttt{components/\allowbreak{}providers/\allowbreak{}TelemetryProvider.\allowbreak{}tsx})}
...
On mount, if \texttt{/\allowbreak{}api/\allowbreak{}sensors} returns null (offline), the provider hydrates from localStorage so the dashboard has data to show immediately.

\section{Request–Response Lifecycle}

\begin{enumerate}[leftmargin=1.5em]
  \item Browser mounts the dashboard; \texttt{TelemetryProvider} calls \texttt{/\allowbreak{}api/\allowbreak{}sensors}.
  \item Next.js server handler calls \texttt{getLatestReading()} in \texttt{lib/\allowbreak{}thingspeak.\allowbreak{}ts}.
