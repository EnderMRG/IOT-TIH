...
\textbf{Challenge:} The model was trained using TensorFlow/Keras 3, which exports a \texttt{model.json} format that differs from what TensorFlow.js (built for Keras 2) expects. Specific incompatibilities included:
\begin{itemize}[leftmargin=1.5em]
  \item \texttt{input\_layers} and \texttt{output\_layers} were flat arrays in Keras 3 instead of nested arrays.
  \item \texttt{inbound\_nodes} used Keras tensor objects instead of flat \texttt{[name, node, tensor]} arrays.
  \item InputLayer used \texttt{batch\_shape} instead of \texttt{batchInputShape}.
  \item LSTM weight paths contained \texttt{lstm\_cell/} sub-scopes not expected by TFJS.
...
\textbf{Solution:} A custom \texttt{patch\_model.js} utility script was developed that reads the exported \texttt{model.json}, applies seven structural transformations to convert it to Keras 2 format, and writes the patched file back. Additionally, the weight loading strategy was changed from TFJS's default name-based matching to manual positional assignment using \texttt{layer.setWeights()}, completely bypassing TFJS's internal naming resolution.
...
\textbf{Solution:} The AttentionLayer was reimplemented in TypeScript as a \texttt{tf.layers.Layer} subclass with the correct \texttt{build()}, \texttt{call()}, and \texttt{computeOutputShape()} methods. The attention mechanism (tanh scoring + softmax weighting + weighted sum) was replicated using TensorFlow.js tensor operations. The layer is registered via \texttt{tf.serialization.registerClass(AttentionLayer)} before model loading, allowing the model JSON to be deserialised correctly.
...
\textbf{Solution:} A \texttt{sample\_data.json} file containing 200 rows from the test set is shipped with the application (\texttt{public/sample\_data.json}). When the live history is shorter than 48 entries, the prediction hook prepends records from the sample data to reach the required window size. This allows the ML inference to operate immediately on new deployments, with the live data gradually replacing the sample data as the session accumulates readings.
...
\textbf{Solution:} All GSAP and Lenis code is guarded with \texttt{"use client"} directives and lazily initialised after component mount using \texttt{useEffect}. The \texttt{useGSAP} hook from \texttt{@gsap/react} handles proper cleanup of ScrollTrigger instances to prevent memory leaks on route changes.
