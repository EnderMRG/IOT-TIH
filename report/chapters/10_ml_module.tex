...
The training dataset is a custom-built synthetic sensor dataset modelled on the Brahmaputra river basin, Assam (\texttt{assam\_flood\_sensor\_data.csv}). The dataset was loaded and processed in Google Colab (\texttt{Model/FloodMonitoring.ipynb}).
...
The dataset is sorted chronologically and indexed by a combined datetime column (\texttt{date + time}). The target variable \texttt{water\_level\_plus\_4h} represents the actual water level 4 hours into the future, constructed from the time-series by a look-ahead shift.

\section{Data Cleaning}

\begin{itemize}[leftmargin=1.5em]
  \item Datetime index constructed by combining the \texttt{date} and \texttt{time} columns using \texttt{pd.to\_datetime()}.
  \item Chronological sorting applied via \texttt{sort\_values('datetime')}.
...
\texttt{sklearn.preprocessing.StandardScaler} is fitted on the training set only and applied to all three splits:

\begin{equation}
  x_{\text{scaled}} = \frac{x - \mu_{\text{train}}}{\sigma_{\text{train}}}
\end{equation}

Separate scalers are fitted for features (X) and target (Y). The scaler parameters are serialised to \texttt{Model/scalers.json} and copied to \texttt{public/model\_scalers.json} for browser-side use.

Scaler parameters (from \texttt{scalers.json}):
\begin{itemize}[leftmargin=1.5em]
...
The \texttt{create\_windows()} function creates overlapping sliding windows of length LOOKBACK = 48 over the scaled feature matrix:

\begin{equation}
  X_i = \mathbf{x}_{i-48:i}, \quad Y_i = y_i \quad \text{for } i \geq 48
\end{equation}

This produces input tensors of shape \texttt{(N, 48, 11)} and target tensors of shape \texttt{(N, 1)}.
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item \texttt{return\_sequences=True} so the full sequence output \texttt{(batch, 48, 128)} is passed to the Attention layer.
\end{itemize}
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item \textbf{Implementation:}
    ...
    \end{align}
  \item Output: context vector $\mathbf{c}$ of shape \texttt{(batch, 128)}.
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item \textbf{Callbacks:}
    \begin{itemize}
      \item \texttt{EarlyStopping(monitor=`val\_loss', patience=10, restore\_best\_weights=True)}: Terminates training when validation loss does not improve for 10 consecutive epochs and restores the best-seen weights.
      \item \texttt{ReduceLROnPlateau(monitor=`val\_loss', factor=0.5, patience=5)}: Halves the learning rate when validation loss plateaus for 5 epochs, aiding convergence.
    \end{itemize}
...
\begin{enumerate}[leftmargin=1.5em]
  \item The model is saved as \texttt{flood\_model.h5} using Keras's legacy H5 format.
  \item The TensorFlow.js converter (\texttt{tensorflowjs\_converter}) converts it to LayersModel format: \texttt{model.json} (topology + weights manifest) and \texttt{group1-shard1of1.bin} (binary weights).
  \item \textbf{Compatibility patching:} Because the model was trained with Keras 3 but TensorFlow.js expects Keras 2 format, the \texttt{patch\_model.js} script performs seven structural transformations:
    \begin{enumerate}
      \item Fix \texttt{input\_layers} / \texttt{output\_layers} nesting.
      \item Convert \texttt{inbound\_nodes} from Keras 3 tensor objects to Keras 2 flat arrays.
      \item Rename \texttt{batch\_shape} to \texttt{batchInputShape} in InputLayer.
      \item Strip LSTM \texttt{lstm\_cell/} weight path sub-scopes.
    ...
    \end{enumerate}
  \item Scaler parameters are exported to \texttt{scalers.json} and model configuration to \texttt{config.json}.
\end{enumerate}

\section{Browser-Side Inference (\texttt{lib/flood-model.ts})}

\subsection{Model Loading}

The \texttt{loadModel()} function:
\begin{enumerate}[leftmargin=1.5em]
  \item Fetches \texttt{model.json}, \texttt{group1-shard1of1.bin}, \texttt{model\_config.json}, and \texttt{model\_scalers.json} in parallel.
  ...
  \item Manually assigns weights to each layer via \texttt{layer.setWeights(tensors)} in positional order, bypassing TFJS internal variable naming.
\end{enumerate}

This manual weight assignment strategy is necessary because of the Keras 3 naming differences (e.g., \texttt{lstm\_cell/} sub-scopes) that survive even after the patch script.
...
\begin{enumerate}[leftmargin=1.5em]
  \item Take the last 48 readings from the live history (padded with \texttt{sample\_data.json} if insufficient).
  \item Call \texttt{engineerFeatures()} to produce a \texttt{(48, 11)} feature matrix.
  \item Wrap in a \texttt{tf.tensor3d} of shape \texttt{(1, 48, 11)}.
  \item Call \texttt{model.predict(inputTensor)}.
