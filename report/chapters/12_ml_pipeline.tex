...
\begin{tikzpicture}[node distance=0.6cm]
  % Training pipeline
  \node[startend, text width=3cm] (dataset) {Dataset\\\texttt{.csv}};
  ...
  \node[process, text width=3cm, below=of eval] (serial) {Serialise\\(\texttt{.h5})};
...
The raw CSV dataset (\texttt{assam\_flood\_sensor\_data.csv}) contains timestamped multivariate sensor readings with a look-ahead water level target. The dataset is loaded into a Pandas DataFrame and sorted chronologically.

\subsection{Stage 2: EDA and Visualisation}
Seaborn and Matplotlib are used to generate the four-panel EDA plot (\texttt{flood\_plots/eda\_plots.png}), providing insight into the temporal distribution, seasonal patterns, feature correlations, and key relationships.
...
The \texttt{prepare\_sequences()} function engineers 11 features from the raw columns: four cyclical time encodings, five raw sensor readings, and two 3-hour trend features.
...
Scalers are fitted on the training split only. Both X (features) and Y (target) are normalised to zero mean and unit variance. Scaler parameters are exported to \texttt{scalers.json}.

\subsection{Stage 7: Sliding Window Creation}
Overlapping windows of length 48 are created over the scaled feature matrix, producing input tensors \texttt{(N, 48, 11)} and target scalars \texttt{(N, 1)}.
...
Four evaluation plots are generated (\texttt{flood\_plots/evaluation\_plots.png}): actual vs predicted on the full test set, zoomed peak event, residuals, and attention weights.

\subsection{Stage 12: Serialisation}
The trained model is saved as \texttt{flood\_model.h5}. Scaler parameters (\texttt{scalers.json}), model configuration (\texttt{config.json}), and sample data (\texttt{sample\_data.json}) are saved.

\subsection{Stage 13: TFJS Conversion and Patching}
The \texttt{tensorflowjs\_converter} converts \texttt{flood\_model.h5} to LayersModel format. The \texttt{patch\_model.js} script patches the resulting \texttt{model.json} for Keras 2 compatibility, fixes weight name paths, and replaces the Orthogonal initialiser.

\subsection{Stage 14: Export Artifacts}
All artifacts are packaged: \texttt{tfjs\_model/model.json}, \texttt{tfjs\_model/group1-shard1of1.bin}, \texttt{scalers.json}, \texttt{config.json}, \texttt{sample\_data.json}. These are copied to \texttt{public/} for browser access.

\subsection{Stage 15: Browser Model Loading}
The \texttt{loadModel()} function in \texttt{lib/flood-model.ts} fetches all artifacts in parallel, manually loads weights, and registers the custom AttentionLayer.
