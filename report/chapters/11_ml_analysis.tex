...
This chapter provides a detailed interpretation of the visualisations generated during the machine learning pipeline. All figures were produced by \texttt{Model/nb\_code.py} (extracted from \texttt{Model/FloodMonitoring.ipynb}) and saved to \texttt{Model/flood\_plots/}.
...
\textbf{Description:} A time-series line plot of \texttt{water\_level\_cm} over the full dataset time range. Red scatter points overlay the time steps where \texttt{flood\_alert\_now = 1}.
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item Red flood alert points cluster at the peaks, confirming that the \texttt{flood\_alert\_now} flag correctly identifies the high-water-level events.
...
\textbf{Description:} A box plot (Seaborn) of \texttt{water\_level\_cm} stratified by \texttt{month\_of\_year}.
...
\textbf{Description:} A Seaborn heatmap showing Pearson correlation coefficients between all numeric features and the target variable \texttt{water\_level\_plus\_4h}, sorted by absolute correlation.
...
\textbf{Description:} A scatter plot of \texttt{pressure\_trend\_3h} (x-axis) against \texttt{water\_level\_plus\_4h} (y-axis), colour-coded by \texttt{month\_of\_year}.
...
\textbf{Significance:} This plot provides evidence that \texttt{pressure\_trend\_3h} is a useful early indicator of flood risk, justifying its inclusion as an explicit engineered feature rather than relying on the raw pressure value alone.
...
\textbf{Description:} A line plot overlaying the actual water level (\texttt{y\_true}) and the model's predicted water level (\texttt{y\_pred}) across the entire test set, both in original (denormalised) centimetre units.
