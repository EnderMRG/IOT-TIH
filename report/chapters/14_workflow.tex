...
\begin{enumerate}[leftmargin=1.5em]
  \item ESP32 powers on and initialises all sensors via \texttt{Wire.\allowbreak{}begin(21,\allowbreak{} 22)} for I\textsuperscript{2}C (BMP180) and the relevant digital pins for DHT11 and HC-SR04.
  \item The \texttt{loop()} function reads all sensor values.
  \item Values are formatted as an HTTP POST payload to ThingSpeak (\texttt{api.\allowbreak{}thingspeak.\allowbreak{}com/\allowbreak{}update}).
...
\begin{enumerate}[leftmargin=1.5em]
  \item Client browser calls \texttt{/\allowbreak{}api/\allowbreak{}sensors} (every \texttt{refreshInterval} seconds).
  \item Vercel serverless function invokes \texttt{getLatestReading()} from \texttt{lib/\allowbreak{}thingspeak.\allowbreak{}ts}.
  ...
  \item \texttt{/\allowbreak{}api/\allowbreak{}history}, \texttt{/\allowbreak{}api/\allowbreak{}analytics} follow the same pattern with 55s TTL.
\end{enumerate}
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  \item Initial API call to \texttt{/\allowbreak{}api/\allowbreak{}sensors}: loading overlay displayed until response.
...
\begin{enumerate}[leftmargin=1.5em]
  \item \texttt{evaluateAlerts(currentData,\allowbreak{} previousData,\allowbreak{} settings)} is called on every new reading.
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  \item \texttt{loadModel()} fetches \texttt{model.\allowbreak{}json}, \texttt{group1-shard1of1.\allowbreak{}bin}, config, and scalers in parallel.
  \item Weights are manually assigned to each layer; model is marked as ready.
  \item Sample data is fetched from \texttt{/\allowbreak{}public/\allowbreak{}sample\_\allowbreak{}data.\allowbreak{}json} for padding.
