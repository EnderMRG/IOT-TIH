...
\begin{tikzpicture}[node distance=0.9cm, every node/.style={font=\small}]
  ...
  \node[process, text width=3.5cm, below=0.5cm of next, xshift=-2.1cm] (apiroute) {Route Handlers\\(\texttt{/\allowbreak{}api/\allowbreak{}*})};
...
The firmware (\texttt{IOTcode/\allowbreak{}IOTcode.\allowbreak{}ino}) initialises the BMP180 via the \texttt{Adafruit\_\allowbreak{}BMP085} library and polls readings in a 2-second loop, printing values to the serial monitor. The full deployment extends this sketch to post field values to ThingSpeak via an HTTP client library.
...
The backend is implemented as Next.js Route Handlers (the App Router equivalent of API routes). All server logic resides in the \texttt{app/\allowbreak{}api/\allowbreak{}} directory. The key components are:

\begin{itemize}[leftmargin=1.5em]
  \item \textbf{\texttt{lib/\allowbreak{}thingspeak.\allowbreak{}ts}:} A server-side ThingSpeak client with an in-memory cache and request coalescing. The latest-reading cache has a 14-second TTL (just under the 15-second upload interval); the history cache has a 55-second TTL. Request coalescing prevents cache stampedes when multiple server renders arrive simultaneously.
  \item \textbf{Route Handlers:} \texttt{/\allowbreak{}api/\allowbreak{}sensors}, \texttt{/\allowbreak{}api/\allowbreak{}history}, \texttt{/\allowbreak{}api/\allowbreak{}analytics}, \texttt{/\allowbreak{}api/\allowbreak{}environment-score}, \texttt{/\allowbreak{}api/\allowbreak{}flood-predict} (placeholder for server-side inference). Each returns a structured JSON response.
...
\begin{itemize}[leftmargin=1.5em]
  \item Polls \texttt{/\allowbreak{}api/\allowbreak{}sensors} at a configurable interval (default 15 s).
...
The ML inference layer runs separately in the \textbf{\texttt{useFloodPrediction}} hook, which loads the TensorFlow.js model from \texttt{/\allowbreak{}public/\allowbreak{}tfjs\_\allowbreak{}model/\allowbreak{}}, engineers the 11 input features from the history array, and calls \texttt{predictFloodRisk()} to obtain a 4-hour water level forecast.
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  \item The Next.js server's \texttt{TelemetryProvider} polls \texttt{/\allowbreak{}api/\allowbreak{}sensors} (every 15 s).
  \item \texttt{/\allowbreak{}api/\allowbreak{}sensors} calls \texttt{getLatestReading()} in \texttt{lib/\allowbreak{}thingspeak.\allowbreak{}ts}, which fetches \\ \texttt{https:/\allowbreak{}/\allowbreak{}api.\allowbreak{}thingspeak.\allowbreak{}com/\allowbreak{}channels/\allowbreak{}\{id\}/feeds/last.json} and parses the five field values into a typed \texttt{TelemetryData} object.
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item \textbf{Environment Variables:} ThingSpeak credentials (\texttt{THINGSPEAK\_\allowbreak{}CHANNEL\_\allowbreak{}ID}, \texttt{THINGSPEAK\_\allowbreak{}READ\_\allowbreak{}API\_\allowbreak{}KEY}) and the MapTiler key (\texttt{NEXT\_\allowbreak{}PUBLIC\_\allowbreak{}MAPTILER\_\allowbreak{}KEY}) are injected as Vercel environment variables, never exposed in the repository.
